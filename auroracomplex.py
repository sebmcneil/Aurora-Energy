...
node_demands = pd.read_csv("hourlydemandbynode.csv", index_col=0)  # 168 hours by 20 demand nodes
# node_demands = node_demands.iloc[:24, :] # to look at first day only
shift_factor_matrix = pd.read_csv("shiftfactormatrix.csv", index_col=0)  # Have LINE_ID as row index of the dataframe
line_ratings = pd.read_csv("lineratings.csv", index_col=0)  # 532x1 vector
...
problem.solve(solver=cp.ECOS, verbose=True)  # Use ECOS solver, verbose to show progress
